  ...
  return (
    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl p-4 sm:p-6 border border-slate-200/50 dark:border-slate-700/50 shadow-sm">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <div>
          <h4 className="text-base sm:text-lg font-bold text-slate-800 dark:text-white">Grafik Pendapatan {currentYear}</h4>
          <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400">Total Omzet Penjualan Per Bulan</p>
        </div>
        <div className="flex items-center space-x-2">
          <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-blue-500 inline-block shadow-md shadow-blue-500/50"></span>
          <span className="text-[11px] sm:text-xs text-slate-600 dark:text-slate-400 font-medium">Pendapatan (Rp)</span>
        </div>
      </div>

      {/* Visualisasi Grafik Batang dengan pengaman scroll horizontal untuk layar kecil */}
      <div className="w-full overflow-x-auto pb-2 scrollbar-none">
        <div className="h-60 sm:h-80 min-w-112.5 flex items-end justify-between gap-2 pt-8 pb-2 px-2 border-b border-slate-200 dark:border-slate-700">
