  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl border border-slate-200/50 dark:border-slate-700/50 p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
          ...
          <button
            ...
            className="flex items-center justify-center space-x-2 px-4 py-2.5 bg-linear-to-r from-blue-600 to-purple-600 text-white rounded-xl shadow-lg shadow-blue-500/25 hover:opacity-95 transition-all text-sm font-semibold whitespace-nowrap"
          >
        ...
        <div className="bg-linear-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20 flex flex-col justify-center relative overflow-hidden">
          <DollarSign className="absolute -right-4 -bottom-4 w-32 h-32 text-white/10" />
          <p className="text-blue-100 text-sm font-medium">Estimasi Beban Gaji (Aktif)</p>
          <h2 className="text-2xl font-black mt-1">
            Rp {totalGajiAktif.toLocaleString('id-ID')}
          </h2>
          <p className="text-xs text-blue-200 mt-2">/ bulan</p>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200/50 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/30">
                <th className="text-left p-4 text-sm font-semibold text-slate-600 dark:text-slate-400">Nama Karyawan</th>
                <th className="text-left p-4 text-sm font-semibold text-slate-600 dark:text-slate-400">Jabatan</th>
                <th className="text-left p-4 text-sm font-semibold text-slate-600 dark:text-slate-400">Gaji Pokok</th>
            ...
            <tbody>
              {filteredEmployees.length === 0 ? (
                <tr>
                  <td colSpan="5" className="p-8 text-center text-slate-500">Belum ada data karyawan.</td>
                </tr>
              ) : (
                  ...
                  <tr key={emp._id} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="p-4">
                      <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 rounded-full bg-linear-to-tr from-blue-500 to-purple-500 flex items-center justify-center text-white font-bold shrink-0">
                          {emp.namaKaryawan.charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <p className="text-sm font-bold text-slate-800 dark:text-white">{emp.namaKaryawan}</p>
                    ...
                    <td className="p-4">
                      <span className={`inline-flex items-center space-x-1.5 px-2.5 py-1 text-xs font-bold rounded-full ${
                        ...
                        {emp.status === 'Aktif' ? <UserCheck className="w-3.5 h-3.5" /> : <UserX className="w-3.5 h-3.5" />}
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 w-full max-w-md shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              <div>
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Nama Karyawan</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Jabatan</label>
                <select
              ...
              <div>
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Gaji Pokok / Bulan (Rp)</label>
                <input
                  type="number"
                  required
                  min="0"
                  ...
                  className="w-full p-2.5 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-800 dark:text-white font-medium"
                  placeholder="Contoh: 3500000"
                />
              </div>

              <div>
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Status Karyawan</label>
