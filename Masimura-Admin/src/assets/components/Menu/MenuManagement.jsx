  ...
  return (
    <div className="space-y-4 sm:space-y-6 pb-6">
      ...
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl border border-slate-200/50 dark:border-slate-700/50 p-4 sm:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
        <div>
          <h3 className="text-lg sm:text-xl font-bold text-slate-800 dark:text-white">Kelola Menu Produk</h3>
          <p className="text-[11px] sm:text-sm text-slate-500 dark:text-slate-400">Daftar Katalog Hidangan Masimura POS</p>
        </div>
        <button
        ...
        <div className="flex items-center space-x-1.5 sm:space-x-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
          <button
            ...
            className={`px-3 sm:px-4 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${
              selectedCategoryFilter === 'Semua' ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200'
            }`}
          >
            Semua
          </button>
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 sm:px-4 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${
                selectedCategoryFilter === cat.namaKategori ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200'
              }`}
            >
              {cat.namaKategori}
            </button>
          ...
          <table className="w-full min-w-175">
            <thead>
              <tr className="border-b border-slate-200/50 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/30">
                ...
                <th className="text-left p-3 sm:p-4 text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-400">Kategori</th>
                <th className="text-left p-3 sm:p-4 text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-400">Harga</th>
                <th className="text-left p-3 sm:p-4 text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-400">Bahan Resep</th>
            ...
            <tbody>
              {filteredMenus.length === 0 ? (
                <tr>
                  <td colSpan="5" className="p-8 text-center text-slate-500 text-xs sm:text-sm">Belum ada menu produk.</td>
                </tr>
              ) : (
                  ...
                  <tr key={menu._id} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="p-3 sm:p-4">
                      <div className="flex items-center space-x-2 sm:space-x-3">
                        ...
                        <div>
                          <p className="text-xs sm:text-sm font-bold text-slate-800 dark:text-white line-clamp-1">{menu.namaMenu}</p>
                          <p className="text-[10px] text-slate-400 uppercase tracking-wider">{menu.kategori}</p>
                        </div>
                      </div>
                    </td>
                    <td className="p-3 sm:p-4">
                      <span className="px-2 sm:px-2.5 py-0.5 sm:py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-full text-[10px] sm:text-xs font-semibold">
                        {menu.kategori}
                      </span>
                    </td>
                    <td className="p-3 sm:p-4">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
            ...
            {/* Modal Header */}
            <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-700 p-4 sm:p-5 shrink-0">
            ...
            <div className="p-4 sm:p-5 overflow-y-auto space-y-3 sm:space-y-4">
              ...
              {/* Nama Menu */}
              <div>
                <label className="text-[11px] sm:text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Nama Menu</label>
                <input
                  type="text"
                  required
              ...
              {/* Upload Gambar */}
              <div>
                <label className="text-[11px] sm:text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Pilih Foto Produk</label>
                <div className="flex items-center space-x-3 sm:space-x-4 mt-1">
                  {imagePreview ? (
                    <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl overflow-hidden border border-slate-200 shrink-0">
                      <img src={imagePreview} alt="Preview" className="w-full h-full object-cover" />
                    </div>
                  ) : (
                    <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl border border-dashed border-slate-300 flex items-center justify-center bg-slate-50 shrink-0 text-slate-400">
                  ...
                  <div className="flex-1">
                    ...
                    <button 
                      ...
                      className="px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-50 text-blue-600 text-[11px] sm:text-xs font-semibold rounded-lg hover:bg-blue-100 transition-colors"
                    >
                      Pilih File
                    </button>
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                {/* Kategori */}
                <div>
                  <label className="text-[11px] sm:text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Kategori</label>
                  <select
                ...
                {/* Harga */}
                <div>
                  <label className="text-[11px] sm:text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Harga (Rp)</label>
                  <input
                    type="number"
                    required
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 p-3 sm:p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                <label className="text-xs sm:text-sm font-bold text-slate-800 dark:text-white mb-2 sm:mb-3 block border-b border-slate-200 dark:border-slate-700 pb-2">
                  Bahan Resep
                </label>
                ...
                <div className="flex gap-1.5 sm:gap-2 mb-3">
                  <div className="flex-1">
                    <select
                      ...
                      className="w-full p-1.5 sm:p-2 text-[11px] sm:text-xs bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-slate-800 dark:text-white"
                    >
                  ...
                  <button
                    ...
                    className="p-1.5 sm:p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors flex items-center justify-center shrink-0"
                ...
                {/* List Resep Terpilih */}
                <div className="space-y-1.5 sm:space-y-2 max-h-32 overflow-y-auto pr-1">
            ...
            {/* Modal Footer */}
            <div className="border-t border-slate-200 dark:border-slate-700 p-4 sm:p-5 flex justify-end space-x-2 sm:space-x-3 shrink-0">
              <button
                type="button"
              ...
              <button
                ...
                className="px-4 sm:px-6 py-2 text-xs sm:text-sm bg-blue-600 text-white font-medium rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-50"
              >
